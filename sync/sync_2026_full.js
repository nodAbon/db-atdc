...
async function syncFromExcelFile(filePath) {
  ...
  for (const r of rows) {
    ...
    if (!dateDigits.startsWith('2026')) continue; // 2026년도 데이터만 필터링
...
async function syncFromMySQL() {
  ...
  try {
    console.log('🔌 MySQL 데이터베이스 연결 시도 중...');
    conn = await mysql.createConnection(MYSQL_CONFIG);
    console.log('🔌 MySQL 연결 성공!');

    // 1. 임직원 동기화
    console.log('\n[1/3] 👥 2026년 재직 임직원 목록 동기화 중...');
    const [empRows] = await conn.execute(`
      SELECT
        ...
        p.N_POSITION AS rank_name,
        pos.N_POSITION AS position_name,
        COALESCE(e.I_RETIRE_YN, '0') AS retire_yn
      FROM hr_employee e
      LEFT JOIN hr_department d ON d.I_COMPANY = e.I_COMPANY AND d.I_DEPT = e.I_DEPT
      LEFT JOIN hr_position p ON p.I_COMPANY = e.I_COMPANY AND p.I_POSITION = e.I_POSITION
      LEFT JOIN hr_position pos ON pos.I_COMPANY = e.I_COMPANY AND pos.I_POSITION = e.I_DUTY
      WHERE e.I_COMPANY = ?
    ...
    const employees = (empRows || []).map((r) => ({
      ...
      rank: String(r.rank_name || '').trim(),
      position: String(r.position_name || '').trim(),
      is_active: r.retire_yn !== '1',
      synced_at: new Date().toISOString(),
    })).filter((e) => Boolean(e.emp_no && e.name));

    await supabase.from('db_employees').upsert(employees, { onConflict: 'emp_no' });
    ...
    for (const c of leaveChunks) {
      await supabase.from('db_leaves').upsert(c, { onConflict: 'emp_no,start_date,leave_code' });
    }
    console.log(`✅ db_leaves 테이블: ${uniqueLeaves.length}건 동기화 완료!`);

    // 3. 2026년 전체 CAPS 출입 기록 동기화
    ...
    if (e.code === 'ETIMEDOUT' || e.message.includes('ETIMEDOUT')) {
      console.log('\n⚠️ [네트워크 알림] 현재 PC에서 사내 AWS MySQL(3306)에 직접 연결할 수 없습니다.');
      console.log('💡 해결 방법:');
      console.log('   1. 사내 네트워크(오피스 Wi-Fi 또는 VPN)에 연결한 후 다시 실행하시거나,');
      console.log('   2. 캡스 엑셀 파일이 있으시면 다음 명령어로 1초 만에 일괄 적재할 수 있습니다:');
      console.log('      node sync/sync_2026_full.js --file="경로/파일명.xlsx"\n');
    } else {
